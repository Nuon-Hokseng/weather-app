...
import { searchCity, defaultCity, WeatherData } from "./data/weatherData";
...
export default function Home() {
  ...
  const [weather, setWeather] = useState<WeatherData>(defaultCity);
  ...
  const handleSearch = (q?: string) => {
    ...
    const result = searchCity(searchQ);
    if (result) {
      setWeather(result);
      setError("");
      setKey(k => k + 1);
    } else {
      setError(`No data for "${searchQ}". Try: London, Tokyo, New York, Paris, Sydney, Dubai`);
    }
  };
    ...
    <main style={{ minHeight: "100vh", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: "2rem 1.25rem 4rem" }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          <div style={{ display: "flex", gap: "0.75rem", marginBottom: "0.75rem" }}>
            <input
              ...
              placeholder="Search city — e.g. Tokyo, London, Dubai..."
              value={query}
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "12px",
                background: "linear-gradient(135deg, #3b82f6, #60b8ff)",
                border: "none",
                color: "#fff",
          ...
          <div className="scroll-x" style={{ display: "flex", gap: "0.5rem", paddingBottom: "0.25rem" }}>
            {SUGGESTIONS.map(city => (
              <button key={city} className="city-pill" onClick={() => { setQuery(city); handleSearch(city); }}>
                {city}
              </button>
            ))}
          </div>
        ...
        <div style={{ textAlign: "center", marginTop: "2.5rem", color: "var(--text-muted)", fontSize: "0.75rem" }}>
          Displaying mock weather data · Built with Next.js
        </div>
      </div>
    </main>
  );
